...
const getnotdoctors = async (req, res) => {
  try {
    const docs = await Doctor.find({isDoctor:false}).populate("userId");
...
const applyfordoctor = async (req, res) => {
  try {
    const alreadyFound = await Doctor.findOne({ email: req.email });
    if (alreadyFound) {
      return res.status(400).send("Application already exists");
    }

    const doctor = await Doctor.create({ ...req.body.formDetails, userId: req.locals });
...
const rejectdoctor = async (req, res) => {
  try {
    ...
    const notification = await Notification({
      userId: req.body.id,
      content: `Sorry ${details.firstname} ${details.lastname}, Your application has been rejected.`,
    });
